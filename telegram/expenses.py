...
def add_expense(raw_message: str) -> Expense:
    ...
    parsed_message = _parse_message(raw_message) # не публичный метод
...
def get_today_statistics() -> str:
    ...
    result = cursor.fetchone()
    #if result >= 50:
    #    print("Привышен лимит. Экономьте")
    base_today_expenses = result[0] if result[0] else 0
    return (f"Расходы сегодня:\n"
            f"всего — {all_today_expenses} руб.\n"
            f"Лимит 50 руб.\n"
            f"Во избежании привышения лимита\n"
            f"Советую экономить")
...
def _parse_message(raw_message: str) -> Message:
    ...
    regexp_result = re.match(r"([\d ]+) (.*)", raw_message) # регулярные выражения
...
def _get_now_datetime() -> datetime.datetime:
    """Возвращает сегодняшний datetime"""
